...
この計算結果の型は`Ordering`なので、3つの構築子`LT`, `EQ`, `GT`のうちのいずれかです。
...
他方で、ほぼ型検査で使うつもりでない限り（詳細はのちほど）、大抵の関数名には小文字始まりの識別子を使います。
...
いくつかの値を関数から`Pair`やより大きなタプルにくるんで返すことはかなりよくあるので、Idrisはいくつかの糖衣構文を提供しています。
...
たとえば、`Weekday`を読み取る関数の例でいうと、後で不正な入力文字列の値を知りたくなることがあるかもしれません。
