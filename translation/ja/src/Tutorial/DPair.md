...
これらの型は依存する値へのパターン合致によってのみ行えることはよくあります。
...
なお、通常の対のように、依存対を作ったりパターン合致したりするのに同じ構文`(x ** y)`を使うことができます。
...
時々、指標値を指標付けられた型の値へのパターン合致で決定することができることがあります。
例えばベクタへのパターン合致により長さの指標を知ることができます。
...
依存対の最初の値にパターン合致することで、2つ目の値がRNAないしDNA配列のどちらになるかを判定できます。
...
これらの符号化の間の損失のない変換を書くことは詰まらないことで、それぞれの符号化があれば1回のパターン合致で、現時点でRNAないしDNAどちらの配列があるのかを決めることができます。
...
演習2から7では、関数が依存対ないしレコードのどちらを返すべきか、関数が追加の引数を必要とすべきかどうか、何にパターン合致できるのか、そして他のどのユーティリティ関数が必要なのか、について自分で決めなくてはいけません。
...
したがってこの数は急増する可能性があり、
このような場合には直和型の符号化は長いパターン合致ブロックに繋がりかねません。
...
スキーマのパターン合致により、型安全なやり方でそれができることに目を向けてください。
...
* 明示的引数でのパターン合致は構文的オーバーヘッドが比較的少ない。
...
これらの値へのパターン合致により型と他の値の取り得る形状についてわかりますが、
