...
*網羅的*な（パターン合致で全ての可能な場合を押さえているような）非再帰関数は、
...
末尾再帰関数を（入れ子になりうる）パターン合致で構成することも可能です。
パターン合致の複数の分岐それぞれで末尾位置での再帰呼び出しがあります。
以下は一例です。
...
というのも、結果が返されるおはパターン合致が`Nil`の場合に到達したときだけだからです。
...
例えばパターン合致されたり他の関数の厳密な引数に渡されたりするときです。
`(&&)`の実装では最初の引数でパターン合致が起こっているので、2つ目の引数が評価されるのは最初の引数が`True`のときだけで、関数の（正格な）結果として返されます。
...
この技法により、とても流麗なコードが導かれることもありますが、`rightFold`が一般にはスタック安全でないことを必ず覚えておいてください。
そのため、累積器が結果を返すまでに、それほど反復が多くないことが保証されない限り、明示的なパターン合致で関数を末尾再帰で実装することをご検討ください。
コードは僅かに冗長になるでしょうが、スタック安全という保証された利点が付いてきます。
